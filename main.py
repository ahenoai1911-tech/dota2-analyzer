...
def format_player_message(data: dict) -> str:
    ...
    t  = data.get("trend", {})
    streak = t.get("streak", {})
    src_icon = "вљЎ" if data.get("source") == "stratz" else "рџ“Љ"
    anon = " рџ”’" if p.get("is_anonymous") else ""

    streak_str = ""
    if streak.get("count", 0) >= 2:
        emoji = "рџ”Ґ" if streak["type"] == "win" else "вќ„пёЏ"
    ...
    for i, h in enumerate(data.get("top_heroes", [])[:5], 1):
        ...
        heroes_lines += f"  {i}. {name} вЂ” {h['matches']}g, WR {h['winrate']}%, KDA {h['kda']}\n"

    matches_lines = ""
    for m in data.get("recent_matches", [])[:5]:
        hero   = m.get("hero") or f"Hero#{m.get('hero_id','?')}"
        result = "вњ…" if m["win"] else "вќЊ"
        matches_lines += f"  {result} {hero} вЂ” {m['kills']}/{m['deaths']}/{m['assists']} ({m['duration_min']}:{m['duration_sec']:02d})\n"

    mmr_str = ""
    if p.get("mmr_estimate"):
        mmr_str = f"\nрџ“Љ MMR: ~{p['mmr_estimate']}"

    return (
        f"{src_icon} <b>{p['name']}</b>{anon}\n"
        f"рџЏ… Rank: {p.get('rank','?')}{mmr_str}\n"
        f"рџ“€ Winrate: {s['winrate']}% ({s['wins']}W / {s['losses']}L)\n"
        f"рџЋ® Total matches: {s['total_matches']}\n"
        f"\nрџ“Љ Trends:\n"
        ...
        f"\nрџ¦ё Top heroes:\n{heroes_lines}"
        f"\nрџ•№ Recent matches:\n{matches_lines}"
        f"\nрџ”— <a href='https://stratz.com/players/{data['account_id']}'>Open on Stratz</a>"
    )
...
async def telegram_webhook(req: Request):
    ...
    if text == "/start":
        ...
        if WEBAPP_URL:
            keyboard = {
                "inline_keyboard": [[
                    {"text": "рџЋ® Open Analyzer", "web_app": {"url": WEBAPP_URL}}
                ]]
            }
        await tg_send(
            chat_id,
            "рџ‘‹ <b>Dota 2 Analyzer</b>\n\n"
            "Send me a nickname or Steam ID and I'll show:\n"
            "вЂў Rank and winrate\n"
            "вЂў Top heroes\n"
            "вЂў Recent matches\n"
            "вЂў Win/loss streaks\n"
            "вЂў KDA and GPM trends\n\n"
            "Or open the Web App рџ‘‡",
        ...
        await tg_send(
            chat_id,
            "рџ“– <b>How to use:</b>\n\n"
            "вЂў Send nickname: <code>Miracle-</code>\n"
            "вЂў Or Steam32 ID: <code>105248644</code>\n"
            "вЂў Or Steam64 ID: <code>76561198065514372</code>\n\n"
            "Data from Stratz API (fallback to OpenDota).",
        )

    elif text.startswith("/"):
        await tg_send(chat_id, "вќ“ Unknown command. Use /help")

    else:
        await tg_send(chat_id, f"рџ”Ќ Looking up <b>{text}</b>...")
        try:
            query = text
            if query.isdigit():
                q_int = int(query)
                ...
                results = await search_combined(query)
                if not results:
                    await tg_send(chat_id, "вќЊ Player not found. Check nickname or ID.")
                    return {"ok": True}
                account_id = results[0]["account_id"]
            ...
            if not result:
                ...
                if not result:
                    ...
                    if not player:
                        await tg_send(chat_id, "вќЊ Profile not found or private.")
                        return {"ok": True}
                    result = build_from_opendota(player, wl, matches, heroes, account_id)
            ...
            if WEBAPP_URL:
                keyboard = {
                    "inline_keyboard": [[
                        {
                            "text": "рџ“Љ РџРѕР»РЅС‹Р№ Р°РЅР°Р»РёР·",
            ...
            await tg_send(chat_id, "вљ пёЏ Error loading data. Try again later.")
