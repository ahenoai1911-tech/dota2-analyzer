...
def init_db():
    ...
    if count == 0:
        ...
        """, [
            ...
            ("daily", "Мастер фарма", "Набери 600+ GPM", "gpm", 600, 75, 120, "💰"),
            ("daily", "Безупречная игра", "Сыграй с KDA 10+", "kda", 10, 80, 130, "⭐"),
            ...
            ("weekly", "Разрушитель", "1M урона башням", "tower_damage", 1000000, 200, 350, "🏰"),
            ("weekly", "Целитель", "50K хила союзникам", "healing", 50000, 180, 300, "💚"),
            ...
            ("monthly", "Богатей", "10000 монет", "total_coins", 10000, 1500, 3000, "💸"),
        ])
        conn.commit()
    ...
    if count == 0:
        ...
        """, [
            ...
            ("Алмазная рамка", "Алмазная рамка", "cosmetic_frame", 800, "💎", "color:diamond"),
            ...
            ("Сброс миссий", "Обновляет миссии", "special_refresh", 300, "🔄", "refresh:all"),
...
# ─── STRATZ ──────────────────────────────────────────────────────────
def stratz_headers():
    return {"Authorization": f"Bearer {STRATZ_TOKEN}", "User-Agent": "Dota2AnalyzerBot/2.5"}

async def stratz_player(aid: int):
    query = """
    query Player($id: Long!) {
        player(steamAccountId: $id) {
            steamAccount { id name avatar isAnonymous seasonRank }
            winCount matchCount
            heroesPerformance(request: { take: 10 }) {
                hero { displayName shortName }
                winCount matchCount avgKills avgDeaths avgAssists
                avgGoldPerMinute avgExperiencePerMinute avgNetworth
            }
            matches(request: { take: 20, orderBy: END_DATE_TIME }) {
                id didRadiantWin durationSeconds endDateTime
    ...
    try:
        async with httpx.AsyncClient(timeout=15) as c:
            r = await c.post(STRATZ_GQL, json={"query": query, "variables": {"id": aid}}, headers=stratz_headers())
            d = r.json()
...
async def stratz_search(nick: str):
    try:
        async with httpx.AsyncClient(timeout=10) as c:
            r = await c.get(f"{STRATZ_BASE}/search", params={"query": nick}, headers=stratz_headers())
            d = r.json()
...
async def search_combined(q):
    ...
    if not res:
        ...
        if od:
            res = [{"account_id": p.get("account_id"), "personaname": p.get("personaname", "Unknown"), "avatarfull": p.get("avatarfull", "")} for p in od]
    return res
...
async def auth(req: Request):
    ...
    c = conn.cursor()
    c.execute(
        "INSERT INTO users (telegram_id, username, last_seen) VALUES (%s, %s, NOW()) ON CONFLICT (telegram_id) DO UPDATE SET username=EXCLUDED.username, last_seen=NOW() RETURNING telegram_id, coins, xp, level",
        (user["id"], user.get("username", ""))
    )
    r = c.fetchone()
    conn.close()
    return {"auth": True, "user": {**r, "tg_username": user.get("username", "")}}
...
async def find_player(query: str = Query(..., min_length=1)):
    ...
    if query.isdigit():
        q_int = int(query)
        aid = steam64_to_account_id(q_int) if q_int > 76561197960265728 else q_int
    else:
    ...
    if STRATZ_TOKEN:
        ...
        if sd:
            ...
            heroes = []
            for h in (sd.get("heroesPerformance", []) or []):
                hero = h.get("hero", {}) or {}
                hm = h.get("matchCount", 1) or 1
                heroes.append({
                    "hero_name": hero.get("displayName", "?"),
                    "matches": hm,
                    "wins": h.get("winCount", 0),
                    "winrate": round(h.get("winCount", 0) / hm * 100, 1),
            ...
            for m in (sd.get("matches", []) or []):
                ...
                matches.append({
                    "match_id": m.get("id"),
                    "hero": ps.get("hero", {}).get("displayName", "?"),
                    "win": (is_rad and m.get("didRadiantWin")) or (not is_rad and not m.get("didRadiantWin")),
            ...
            result = {
                ...
                "profile": {
                    ...
                    "rank": rank_name(sa.get("seasonRank")),
                    "is_anonymous": sa.get("isAnonymous", False),
                },
                "stats": {"wins": win, "losses": total - win, "total_matches": total, "winrate": round(win / total * 100, 1)},
    ...
    if not result:
        ...
        matches_out = []
        for x in (m or [])[:20]:
            is_rad = x.get("player_slot", 0) < 128
            matches_out.append({
                "match_id": x.get("match_id"),
                "hero": "",
                "win": (is_rad and x.get("radiant_win")) or (not is_rad and not x.get("radiant_win")),
                "kills": x.get("kills", 0),
                "deaths": x.get("deaths", 0),
                "assists": x.get("assists", 0),
                "kda": calc_kda(x.get("kills", 0), x.get("deaths", 0), x.get("assists", 0)),
                "gpm": x.get("gold_per_min", 0),
                "tower_damage": x.get("tower_damage", 0),
                "healing": x.get("hero_healing", 0),
            })
        result = {
            "source": "opendota",
            "account_id": aid,
            "profile": {
                "name": pd.get("personaname", "?"),
                "avatar": pd.get("avatarfull", ""),
                "rank": rank_name(p.get("rank_tier")),
                "is_anonymous": False,
            },
    ...
    if (not result.get("recent_matches") or len(result["recent_matches"]) == 0) and result["stats"]["total_matches"] <= 1:
        raise HTTPException(403, "Private profile or no match data")
...
async def assign_missions(req: Request):
    ...
    if not missions:
        ...
        new = c.fetchall()
        for m in new:
            c.execute("INSERT INTO user_missions (telegram_id, mission_id) VALUES (%s,%s)", (tid, m["id"]))
        conn.commit()
        c.execute("SELECT m.*, um.progress, um.completed, um.claimed FROM missions m JOIN user_missions um ON m.id=um.mission_id WHERE um.telegram_id=%s AND um.assigned_at::date=%s", (tid, today))
        missions = c.fetchall()
    conn.close()
...
async def claim_mission(req: Request):
    ...
    row = c.fetchone()
    if not row:
        raise HTTPException(400, "Already claimed or not completed")
...
async def sync_missions(req: Request):
    ...
    for m in active:
        prog = m["progress"]
        if m["requirement"] == "matches":
            prog = len(matches)
        elif m["requirement"] == "wins":
            prog = sum(1 for x in matches if x.get("win"))
        elif m["requirement"] == "gpm":
            prog = max((x.get("gpm", 0) for x in matches), default=0)
        elif m["requirement"] == "kda":
            prog = max((x.get("kda", 0) for x in matches), default=0)
        elif m["requirement"] == "assists":
            prog = max((x.get("assists", 0) for x in matches), default=0)
        elif m["requirement"] == "tower_damage":
            prog = max((x.get("tower_damage", 0) for x in matches), default=0)
        elif m["requirement"] == "healing":
            prog = max((x.get("healing", 0) for x in matches), default=0)
...
async def ai_chat(req: AIReq):
    ...
    system = "You are an expert Dota 2 coach. Give concise, actionable advice based on stats. Use bullet points. Respond in user's language. Keep under 300 words."
    ...
    async with httpx.AsyncClient(timeout=30) as c:
        r = await c.post(
            ...
            json={"model": "llama-3.3-70b-versatile", "messages": [{"role": "system", "content": system}] + msgs, "max_tokens": 1000, "temperature": 0.7}
        )
    if r.status_code != 200:
        raise HTTPException(502, f"AI failed: {r.text[:100]}")
    return {"reply": r.json()["choices"][0]["message"]["content"]}

@app.post("/roast")
async def roast_player(req: RoastReq):
    if not GROQ_API_KEY:
        raise HTTPException(503, "AI not configured")
    modes = {
        "toxic": "You are a BRUTAL Dota 2 roaster. Roast their stats HARD with dark humor. Short punchy lines with emojis. Under 200 words. Write in Russian. End with a devastating verdict.",
        "friendly": "You are a friendly Dota 2 comedian. Roast gently with humor. Be funny but supportive. Under 150 words. Write in Russian.",
    ...
    system = modes.get(req.mode, modes["toxic"])
    async with httpx.AsyncClient(timeout=30) as c:
        r = await c.post(
            "https://api.groq.com/openai/v1/chat/completions",
            headers={"Authorization": f"Bearer {GROQ_API_KEY}", "Content-Type": "application/json"},
            json={"model": "llama-3.3-70b-versatile", "messages": [{"role": "system", "content": system}, {"role": "user", "content": f"Roast this Dota 2 player:\n{req.player_context}"}], "max_tokens": 800, "temperature": 0.9}
        )
    if r.status_code != 200:
...
async def tg_send(cid, text, kb=None):
    if not BOT_TOKEN:
        return
    async with httpx.AsyncClient(timeout=10) as c:
        await c.post(f"https://api.telegram.org/bot{BOT_TOKEN}/sendMessage", json={"chat_id": cid, "text": text, "parse_mode": "HTML", "reply_markup": kb})
    ...
    if txt == "/start":
        ...
        try:
            ...
            if not aid:
                ...
                if not res:
                    await tg_send(cid, "❌ Не найден.")
                    return {"ok": True}
                aid = res[0]["account_id"]
            r = get_cache(f"player:{txt}")
            if not r:
                r = await find_player(txt)
